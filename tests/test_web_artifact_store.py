...
def _store(
    tmp_path: Path, clock: FrozenClock | None = None, *, limits: ArtifactLimits | None = None
) -> tuple[ArtifactStore, FrozenClock]:
    current_clock = clock or FrozenClock()
    store = ArtifactStore(tmp_path / "artifacts", clock=current_clock, limits=limits)
    store.initialize()
