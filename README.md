...
工具集合、其他插件、基础配置及适配器等变更仍交给 Linux 后台控制器执行完整 Bot 重启、健康检查和失败回滚，只重启 Bot、不重启 LLBot。控制器会验证已热生效的配置而跳过重复重启；未安装控制器时，不支持的变更会在写入前明确拒绝。模型热更新失败会回滚原配置文件，无法确认恢复时明确报错，不把失败显示为保存成功。

分模型的 `api_key`、`base_url` 留空或删除即可继承全局设置。凭证使用环境变量引用，不在表单中填写新密钥明文；新增或轮换进程环境变量仍需先重启使其进入运行环境。管理面板继续仅通过回环地址和 SSH 隧道访问。
