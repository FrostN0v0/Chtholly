...
收藏只接受 JPEG、PNG 与 WebP，文件以无覆盖的纯数字名称写入 `resources/image/memes`，内容哈希用于目录内去重；自动标签立即写入 `chat_image_tags`，因此新图无需重启即可被 `send_image` 检索。图片文件属于可审阅的仓库静态资源，标签数据库仍是运行期本地数据；该能力不新增配置项，GIF 与其他格式会明确拒绝。
