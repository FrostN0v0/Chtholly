...
聊天以可读性选择形态：短答一条，多要点或步骤自然分条，内容多或多段较长时用合并转发包成群聊记录，每个节点保留一个可读段落。既不凑条数，也不为了少发消息压成一堵文字墙；关系更亲近不意味着每轮都要主动追问。普通最终文本的分段超过气泡额度时会整体转为合并转发，不机械切碎原句；不支持转发的平台按顺序回退普通消息。
