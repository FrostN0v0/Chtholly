...
- `llm_chat` 以可读性选择回复形态：短答一条，多要点、步骤或理由按自然段使用多个 `send_text`，内容多或多段较长时在第一次文本发送前使用一次 `send_merged_forward`，每个节点保留一个可读段落及原有顺序。既不凑条数，也不得为了少发消息把长回复压成一堵文字墙；关系亲近不等于强行追问。普通最终文本保留原有分段，超长自然段只按完整句子边界分组，不按字符硬切原句或 URL；尚未开始文本交付而段数超过普通额度时，复用原生合并转发发送路径，不再合回超长单条。已开始发送后仍不得切换模式，节点、总字符及交付额度不变。非 OneBot 或明确不支持转发时按顺序回退普通消息，未知传输失败不盲目重发，失败和取消只保存已确认前缀。需要真实艾特时使用 `send_text`；现有群级调度及引用归属不变。代码、表格和复杂排版保持结构，继续遵循渲染或可复制源码规则；普通多段文字本身不要求渲染成图片，所有媒体仍先于文字交付。
