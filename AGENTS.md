...
- `llm_chat` 关系 evaluator 默认每个用户每 5 次成功回复运行一次，`LLMChatConfig.eval_every_n` 与生产 `entari.yml` 必须保持为 `5`；不得在功能测试外改回逐轮评估。按需历史图片识别继续使用 `image_tag_model`，但视觉调用必须显式 `max_retries=0`，避免单次超时被 SDK 自动重试放大。
...
- `llm_chat` 表情选择去重：`send_image` 的语义与随机检索必须先排除当前频道 recent window 内已发送的资源，仅当图库没有任何未近期发送资源时才允许回退复用；“别的”“换一张”“不同的”等更换意图按随机换图处理。模型收到明确换图要求时必须使用 `context` 重新检索，禁止用 `image_paths` 指回最近图片或在同一轮重复同一路径。带清晰文字的表情必须在结构化 `text` 字段保留关键原文，在 `meaning` 与 `tags` 中记录具体玩梗含义和 `文字表情包`，不得只按人物外观或泛化情绪标注。
- `llm_chat` 表情标签结构：`ImageTag.tags` 固定持久化为单行 JSON，字段为 `text`、`meaning`、`use_when`、`avoid_when` 与 `tags`；`text` 保存图片可见原文，embedding 与正向检索只使用 `text` / `meaning` / `use_when` / `tags`，`avoid_when` 只作发送前硬排除。运行时继续只读兼容旧逗号标签；超管通过 `llmchat retag-memes-legacy` 仅重标旧格式记录。表情管理 WebUI 默认使用简体中文，并分别展示、筛选结构化与旧格式记录。
