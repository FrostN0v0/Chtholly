...
- `llm_chat` 表情包收藏：仅在当前已触发会话内允许模型通过 `tag_image` 收藏本轮顶层直接图片或已补全引用消息中的顶层图片，候选顺序固定为 direct-first / quoted-second，合并转发图片不得进入候选。自动收藏必须排除裸 marker、普通或敏感图片与用户明确拒绝保存的图片；超管可用 `llmchat tag-meme` 附单图或引用单图执行人工覆盖，普通成员也必须收到非空拒绝以确保命令被 claim。导入仅接受 JPEG / PNG / WebP，原始字节经 6 MiB 边界与 MIME 嗅探后，以 SHA-256 去重并通过同目录临时文件 + no-clobber hard link 写入 `resources/image/memes`；标签立即 upsert 到 `chat_image_tags`，embedding 失败时保留标签并依赖 IDF fallback。文件与数据库提交必须在进程内异常及取消时补偿，工具结果不得泄露路径、标签、哈希或数据库信息。每次成功导入或去重命中必须按频道持久化内部收藏确认记录，提示历史只能暴露不含路径与标签的确认语义；`send_image` 的选择优先级固定为用户明确给出的已注册相对路径、当前频道最近确认收藏、普通语义检索，后续“发出来”或“再发一次”不得在工具未报告丢失前改口声称收藏失败。
