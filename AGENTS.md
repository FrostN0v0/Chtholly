...
- 公网 SSO 会话不得使用一小时硬过期且丢弃刷新令牌的 Cookie 模式。当前 OAuth2 Proxy 请求 `offline_access`，使用独立鉴权回环 Redis 保存加密会话，`cookie_refresh=5m`、`cookie_expire=12h`，实际续期仍受 Casdoor 令牌寿命与撤销约束；浏览器仅携带签名票据。外层网关和内层 `/oauth2/auth` 共享同一服务端记录，避免旧 Cookie 在并发资源请求中重复兑换轮换令牌；OAuth2 Proxy 7.15.4 的刷新锁租期只有两秒且无全请求刷新超时，不得把普通并发验收推广成任意慢上游下的原子性保证。退出必须先调用固定回环 `/oauth2/sign_out` 删除服务端会话，再删除两层 Cookie；未确认撤销时不得返回成功。隐藏标签页的 401 只记录待认证，重新可见时访问当前管理路径而不是强制 `/oauth2/start`，从而复用其他标签页的新会话；BFCache 恢复必须清除旧导航门闩并重新检查。静态资源纳入 OAuth2 Proxy 的 `api_routes`，未认证只返回 401，禁止资源请求额外签发 CSRF Cookie；CSRF 登录窗口为一小时，nonce、PKCE、Cookie 签名和同源写入边界不变。
