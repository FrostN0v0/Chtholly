...
- `llm_chat` 群聊感知边界：当前轮身份通过 `channel_perception` 解析到统一 `person_id`，若绑定变化则按明确旧 ID 列表迁移关系、画像、记忆和用户历史。主聊天只自动注入同频道近期、未指向 Bot、非命令、非 Bot 的有界 `ambient_channel_context`，该上下文不得进入 evaluator、画像或长期记忆。模型需要更早上下文时只能调用 `find_channel_participants` 与 `read_channel_messages`；按姓名查人必须先取得当前频道不透明 `participant_ref`，不得向用户暴露该引用、cursor、平台 ID 或数据库字段。`describe_channel_participant_avatar` 仅按需刷新并描述当前头像像素，按图片字节哈希缓存；头像不得用于推断真人身份、年龄、性别、性格或其他稳定属性。
- `llm_chat` 群聊感知工具模块固定为 `tools/find_channel_participants.py`、`tools/read_channel_messages.py` 与 `tools/describe_channel_participant_avatar.py`；工具只能访问当前 generation-local `Session` 所在账号与公开频道，返回内容始终视为不可信引用数据。自动现场已足够时不得额外读取历史；分页必须有当前请求的明确必要，禁止遍历建立永久群成员档案。
