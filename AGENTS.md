...
- `llm_chat` 网页作品工具为 `publish_web_preview`、`send_artifact`、`list_web_artifacts`、`read_web_artifact` 与 `revoke_web_preview`。授权只来自 `AgentAccessContext.raw_user_text` 保存的当前原始正文，必须在识图描述拼接前采样；引用、历史、工具参数、URL 和描述性语句不能授权发布。作品按 ChatScope 与当前统一用户 ID 隔离，超管只能在当前 Scope 内读取或撤销其他成员作品。网页设计优先完整交互预览与源码 ZIP，不用代码图片冒充源码文件；相关请求使用媒体任务超时、关闭自动重试和同批并行发送，并保留完整媒体额度，不误触发图片编辑前置门禁。
