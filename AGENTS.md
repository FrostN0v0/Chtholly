...
- `llm_chat` 出站由 `utils/group_delivery_core.py` 的纯调度队列和 `group_delivery.py` 的原生运行边界按 `platform + account_id + channel_id` 协调，模型、搜索与媒体准备仍按成员并行；只在真正可发送时选择就绪消息，不为尚未提交或延迟未到的续条占位。每组最多连续 3 条或自首条开始 4 秒，达到边界时让其他就绪轮次公平轮转；共用原有规范化节拍，缺少现成续条立即让位。组首、其他成员插话、无关插件出站或长间隔恢复时，用认领时固定的原入站消息 ID 添加原生 `Quote`，不让模型提供 ID，不额外艾特，也不把引用标记写进 assistant 正文历史。`reply_payload.py` 避免 OneBot 语音、视频、文件和合并转发被拆出空引用：保持原生载荷，后续正常文字或图片重新引用；整轮仅含这些媒体时，在成功收尾且所有媒体完成后补一条计入文字额度和真实历史的引用归属提示。实际发送限时 30 秒，排队取消不算发送尝试，传输取消或超时保留未知结果且不重发；轮次结束拒绝迟到子任务发送，卸载取消等待者、传输及定时器，旧运行边界清理不得移除替换边界。
...
- `llm_chat` 人格作息使用 `Asia/Shanghai`，`energy_at` 只接受带时区的时间，每轮仅采样一次供 prompt 与审计使用。精力和关系影响自然表达，不生成档位或缩减规范化交付额度（媒体默认 6 条）；是否回应由模型决定，合法拒绝只发送拒绝文字。`html2pic` 预览与 `markdown2pic` 源码图是不同交付项，`md2pic` 只映射既有 `markdown2pic`，不新增别名。命名渲染与自然语言转图片复用本轮媒体意图判定；媒体先于文字，MIME、6 MiB、渲染及网页安全边界不变。
...
- `llm_chat` 恢复自然多条回复：只有一个短而完整且不需要真实艾特的聊天气泡时，才直接使用最终普通文本；回答有两个以上自然独立的文字节拍时优先调用多个 `send_text`，事实问答和严肃求助同样可将结论、理由或限制、必要建议分开发送，不因它们属于同一个答案就塞成一条长消息。不要切碎句子、机械每句一条或凑满消息额度；关系亲近不等于强行追问。需要真实艾特时即使只有一条也使用 `send_text`。普通最终文本继续按换行拆成独立消息并复用原有额度、节拍与确认前缀；运行时不因段落长度自动改成合并转发。复杂 Markdown 表格、多列对比或长结构化排版继续复用 `markdown2pic`，自定义卡片用 `html2pic`，固定报告用 `jinja2pic`；需要复制的代码、工具缺失或渲染超限时保留文字或 `send_merged_forward`。合并转发不承载本轮艾特，所有渲染图片先于文字并计入原有媒体额度。群级就绪调度与原生引用保持独立，不随回复格式策略回退。
...
- `llm_chat` 同批工具由原生暂停/继续桥统一结算：网页和查询等只读工具可并发，发送、写入、参考签发与 `finish_turn` 按模型顺序串行。明确媒体请求及纠正仍传 `parallel_tool_calls: false`；不得放宽媒体先于文字。合法终止后，余下调用必须写入匹配的未执行结果与终态审计，不能丢失 tool result 或执行后续副作用；拒绝的 `finish_turn` 不得结束生成。部分或未知交付不能被后续空文本包装成完成。
...
- `llm_chat` 原生图片输出：OpenAI-compatible Chat Completions 的 `message.images` 必须在本地 Agno 兼容边界中保留，并统一收窄为经 6 MiB、JPEG / PNG / WebP / GIF MIME 嗅探或公开 URL 策略验证的图片；拒绝任意本地路径。Agno 2.9 的非流式路径不能可靠地将这些图片传入 `RunOutput.images`，因此 `native_image_delivery.py` 用 attempt-local 缓冲保存当前主模型响应，按模型实例隔离嵌套调用，不从历史 `image_output` 回收图片，也不把像素放回后续模型消息。`chat_handler` 绑定既有 `ActiveChatTurn.deliver_model_images`：本批后续发送工具或合法的 `finish_turn(delivered)` 前先交付候选图片，无工具回复则在本次生成返回前完成交付；只有真实确认后才允许结束，不放宽确认计数。全部候选原子预留媒体额度，逐张确认后独立写入 `[发送了图片]` 历史 marker；缓冲在发送前消费，最终化不得重发。部分失败或取消立即终止后续工具并保存匹配结果和已确认前缀，不追加模型重试或 evaluator；合法沉默、拒绝及要求真实参考编辑的轮次不得偷发原生候选。安全图片交付不触发空回复 finalizer 或虚假媒体纠正，工具声明继续来自原生注册。
