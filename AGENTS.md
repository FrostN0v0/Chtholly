...
- `llm_chat` 使用 `default_persona` 和 `personas` 配置多个角色；每个 `PersonaConfig` 包含 `name`、`prompt`、`appearance` 与可选的 `reference_image`。旧顶层 `persona` / `self_reference_image` 已移除，配置迁移必须保留原人格文本、口吻与对应参考图。通用 `SYSTEM_SCAFFOLD` 不得包含固定角色身份、服饰、撒娇口吻或媒体偏好。参考图仅在当前角色相关的明确生图请求中注入，不进入聊天历史或用户图片收藏；路径继续受 `resources/image` 边界与现有图片校验约束。
