...
- `llm_chat` 表情选择与去重：`send_image.context` 只允许正向情绪、场景和主体关键词，不得混入“不要”等排除词、目录名或内部路径；上下文中逐项出现的精确结构化标签必须先收窄候选，只有没有精确标签命中时才使用 embedding 与 IDF 检索，避免宽泛语义覆盖用户点名的梗。检索先确定匹配候选集合，再排除当前频道 recent window 内已发送资源，仅当该匹配集合没有未近期发送资源时才允许回退复用；“别的”“换一张”“不同的”等更换意图按随机换图处理。模型收到明确换图要求时必须使用 `context` 重新检索，禁止用 `image_paths` 指回最近图片或在同一轮重复同一路径。带清晰文字的表情必须在结构化 `text` 字段保留关键原文，在 `meaning` 与 `tags` 中记录具体玩梗含义和 `文字表情包`，不得只按人物外观或泛化情绪标注。
