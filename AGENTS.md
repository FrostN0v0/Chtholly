...
- `plugins/webui_config_apply` 为 Linux 托管部署接管 WebUI 的两个原生配置 PUT 入口，并提供应用状态与完整 Bot 重启按钮；它必须加入既有空前缀的本地插件加载列表，不默认启用在未安装 systemd 控制器的本机环境。`utils/webui_config_core` 在无框架副作用的边界恢复当前源文件的环境引用、规范化分模型继承与校验配置，不复用 Entari 的旧位置插值记录。保存先准备分离的配置副本，写入后仅发布新的持久化容器与新插值记录，不原地修改已加载插件的配置或模型对象；Entari 在卸载插件后还会执行退出保存，因此必须保证该次回写保留刚保存的配置，不能只修 WebUI 的写文件动作。特权控制器源码位于 `scripts/chtholly_config_apply.py`，通过固定的 root-owned 请求文件接收重启指令；配置回写保留文件 inode，运行摘要和已加载插件验证通过后才更新 last-good，失败回滚并保留期间的新保存。所有管理入口复用认证与同源限制，控制器不可用时保存明确失败，凭证不得进入响应、日志或明文 YAML。
...
- 作品原文只保存在 LocalData 的 `llm_chat/web_artifacts` 中；`utils/web_artifacts_core` 使用独立 SQLite 目录保存不可变版本、权限、过期与容量状态，源码 ZIP 必须与已发布文件逐字节一致。项目版本高水位独立于已删除版本，发布与崩溃恢复共享写事务边界，未登记目录残留也计入磁盘额度。`llm_chat.web_artifacts.service` 在 Launart preparing 阶段初始化并清理过期作品，关闭时必须等待正在进行的存储操作与取消补偿。
...
- 作品审计只保留引用、版本、大小、哈希、过期时间与真实副作用状态，不回放源文件映射、base64、公开能力链接或内部截图密钥。源码使用 `File.of(raw=..., mime="application/zip", title=...)` 内联发送，确认后才保存文件历史 marker；能力缺失的链接回退只计入真实文字交付。未知传输失败不得盲目回退重发，取消发生在发布提交后时须等待存储结束并撤销未交付作品，已发生的持久化副作用仍在 AgentEvent 中保留。
...
13. **分支纯净性**：所有变更基于 `master` 新建功能分支，命名使用 `feat/xxx`、`fix/xxx` 或 `chore/xxx`，不要以现有主分支名作为路径前缀；在功能分支按原子性提交规范落成 `commit`，提交信息使用中文并遵循 `gitmoji` 规范。`master` 为唯一应用主线，`resources` 独立保留；清理功能分支不等于允许删除长期分支、替换其为归档标签或改变资源布局。
