...
- **Bot 框架**: [arclet-entari](https://pypi.org/project/arclet-entari/)（基于官方 `0.19.0rc2`，当前安装仓库内的 `0.19.0rc2+chtholly.2` 补丁 wheel，完整保留 `arclet-entari[full]`，含 CLI、YAML、文件监听）。补丁修复 staged reload 对象所有权、Scope 冲突、失败模块绑定恢复、子插件/Service 清理与重复替换后的命令残留；失败候选须恢复旧订阅者持有的真实 Alconna 解析器和 formatter 条目。`scripts/build_patched_wheel.py --package entari` 从固定官方 wheel 与仓库补丁重建制品；同一构建器的 `--package webui` 生成 `1.0.3+chtholly.3`，保留认证 Cookie 与每个聊天连接 transport ID 分离、连接独立清理及认证初始化重试，并加入中文管理主题，保留 Entari Logo 原色。不直接修改第三方安装目录。HTMLRender `0.1.0` 的 Entari 上界通过精确 uv override 保留 `full,pydantic`，升级必须验证真实渲染及失败更新、重复替换和最终卸载。
- **WebUI 视觉边界**: `patches/webui-theme/native.css` 与 `native.js` 仅调整锁定原生 SPA 的外观、导航辅助和扩展主题同步；构建器从固定官方 wheel 应用既有生命周期补丁，再嵌入内容哈希命名的主题文件，不修改供应商压缩 JS 的业务逻辑。`utils/webui_theme/tokens.css` 是原生与三个扩展页的唯一配色色板，使用现代浏览器的 `light-dark()`；扩展页通过 `themed_assets` 在原有 nonce/CSP 内合并本地资源。父页只向同源已挂载 iframe 发送 light/dark，opaque iframe 必须校验父窗口和精确 origin，独立打开时跟随系统主题；不得放宽 sandbox、认证、CSP 或以匿名资源恢复样式。上游升级须复核 DOM 类名、登录、原生设置、会话/表情/工坊、主题切换和 390px 窄屏，不新增第二套管理 API。
...
- `generate_image` 通过独立 `image_generation_model` 创作新图，可选本轮 `reference_image_refs` 与角色参考；没有用户源图也能使用已核验网页原图创作。有像素输入走 LiteLLM Images Edit API，无像素走 Images API，供应商接口名称不决定业务是否编辑。`edit_image` 必须使用 `source_image_ref`，第一张输入固定为该原始快照，其后才是视觉参考与选用的角色图；直接、普通引用、授权转发、频道历史和头像都可作源图，网页和角色图仅可作参考。两类工具不追加固定身份替换或保留构图的生图前缀，原样传递规范化后的任务提示词；主模型尽量保留用户原语言和视觉要求，只补充图片顺序与本次用途，不把参考图重新猜测成详细绘图规范。默认尺寸和 `image_generation_quality` 为 `auto`，在请求中省略，而不是传入字符串 auto；不强制 `input_fidelity` 或响应格式。明确指定的受支持尺寸与质量仍原样请求，不静默改档、不为结果不佳自动重试，也不把请求值当成供应商已执行的事实。图片字节不缩放、抠图或补底色；原图权限、输入顺序、超时、6 MiB、SSRF 及确认交付边界保留。旧个人配置不自动覆盖，启用自动质量需显式将既有值改为 auto，并同步部署运行代码与本地化 schema。
...
- `llm_chat` 历史工具上下文重建使用上游 `available_functions` 的真实 `Function.parameters`，通过显式依赖 `jsonschema` 检查压缩后的参数及嵌套结构，并保持原生执行边界的顶层字段限制。只有整批参数仍符合当前声明时，才将连续调用合并为一个 assistant `tool_calls`，随后按真实完成顺序附上匹配结果；统计摘要、脱敏媒体或成员引用、大段源码描述符及已卸载工具统一作为明确不可执行的 `historical_tool_batch` 文本记录，保留调用顺序、完成顺序、真实状态与原文读取描述符，不伪造调用参数、不恢复旧能力引用。旧记录在读取时处理，不改写数据库或私有审计，不放宽当前工具参数校验。没有匹配结果、重复 ID 或跨轮错配的调用继续省略，不得形成孤立 tool result；审计原始顺序、按完整 Turn 的预算选择及精确原文读取授权保持不变。
...
- `llm_chat` 会话管理 WebUI：`agent_webui.py` 注册 `/extension/llm-chat-sessions` 与同源认证 API，`agent_admin.py` 负责 Session / Turn / AgentEvent 时间线、完整载荷分页、Context Inspector、handoff、anchor、rollover 与 hard reset 管理，静态页面位于 `plugins/llm_chat/webui_sessions/`。中文界面采用顶部频道/会话选择与轮次、详情双栏，窄屏切换列表和详情；默认展示用户输入及按真实回执排序的回复，调用、上下文、关系分别查看，说明与原始记录按需展开。轮次默认最新在上，可切换顺序且不改变选中项；手动刷新保留选中轮次。Token 自动换算 K/M，详细统计提供精确值，未知不等于零、部分统计保持标记，缓存和推理子项不重复相加。工具卡片按 execution_ref 关联实际交付而非推断模型返回。生成期间自动刷新按完整记录路径保留嵌套展开、焦点与滚动位置；完整载荷按需分页读取。耗时区分真实接收至最后确认发送、运行中已过时间、旧版生命周期估算与未记录。Context Inspector 保留该轮实际选择、预算及 baseline 快照，不能用当前配置重算。管理操作不得删除 AgentEvent 审计，hard reset 只能封存旧 Session 并创建无继承的新 Session。
- 会话详情使用有边界的消息卡片和实色对话框，页内不再重复显示独立标题栏；刷新、同步状态、频道与会话选择共用紧凑工具栏。嵌套 user JSON 在管理投影中还原正文、引用归属和提及；列表在 JSON 安全提取后才限幅，不截断封装后展示乱码。`content.js` 只以安全 DOM 展示 Markdown、宿主独占一行的 `【分节标题】`、代码、表格及结构化工具数据，原始载荷按需分页并可切换查看，不执行用户 HTML 或加载 Markdown 远程图片。`charts.js` 将已成功评估的前后值与有符号增减合并到唯一五轴图；无成功结果时只显示生成前快照，未知不画成零。Token 输入/输出与缓存/推理子项分离。部署必须同时包含 `agent_webui_api.py`、`utils/webui_theme` 的共享色板/主题同步、全部 CSS/JS 辅助文件及管理投影；不能只同步页面 CSS 后以健康接口代替实色弹窗、引用消息、单图比较和窄屏验收。
