...
- `plugins/webui_config_apply` 为 Linux 托管部署接管 WebUI 的两个原生配置 PUT 入口，只保留后台安全保存、控制器状态与受认证重启 API，不注入前端脚本、常驻浮窗或独立重启菜单；它必须加入既有空前缀的本地插件加载列表，不默认启用在未安装 systemd 控制器的本机环境。`utils/webui_config_core` 在无框架副作用的边界恢复当前源文件的环境引用、规范化分模型继承与校验配置，不复用 Entari 的旧位置插值记录。保存先准备分离的配置副本，写入后仅发布新的持久化容器与新插值记录，不原地修改已加载插件的配置或模型对象；Entari 在卸载插件后还会执行退出保存，因此必须保证该次回写保留刚保存的配置，不能只修 WebUI 的写文件动作。特权控制器源码位于 `scripts/chtholly_config_apply.py`，通过固定的 root-owned 请求文件接收重启指令；配置回写保留文件 inode，运行摘要和已加载插件验证通过后才更新 last-good，失败回滚并保留期间的新保存。所有管理入口复用认证与同源限制，控制器不可用时保存明确失败，凭证不得进入响应、日志或明文 YAML。
- Entari `0.19.0rc2` 的普通插件、`plugin_config(..., bind=True)` 配置和具备正确清理的 Service 插件均可热重载，不能仅凭存在 Service 就判定必须重启。当前 WebUI `1.0.3` 的原生保存设置 `save_flag` 而不发布 `ConfigReload`；LLM `0.2.2` 的配置监听器仅更新 `models`、`prompt`、`tools`，遗漏全局 `api_key` / `base_url`，删除模型也不修复持久化默认选择。改造保存生效流程时必须贯通配置事件、完整运行时字段、默认模型状态与动态工具生命周期，并验证在途轮次一致性和失败回滚；这些路径未验证前保留后台整进程应用。隔离生命周期实验必须使用真实 `it(Launart)` 容器，并将 Letoderea effect 任务绑定到当前事件循环，避免错误容器或跨 loop 清理制造假失败。
