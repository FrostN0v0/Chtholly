...
- **Satori 服务鉴权**: `server.token` 保护 Satori 根 HTTP / WebSocket 接口；OneBot 适配器的 `access_token` 只保护对应适配器连接，二者不得混用。Entari WebUI 在 loopback 本地模式下跳过内建登录鉴权，因此生产管理面必须保持 `127.0.0.1` 监听并通过 IAP SSH 隧道访问。
