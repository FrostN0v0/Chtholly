...
- `llm_chat` 群聊感知边界：当前轮身份通过 `channel_perception` 解析到统一 `person_id`，若绑定变化则按明确旧 ID 列表迁移关系、画像、记忆和用户历史。主聊天不再自动注入群聊现场；模型只在当前请求确实依赖同频道近期或更早消息、人物称呼或话题衔接时调用 `read_channel_messages`，当前消息与普通会话历史已足够时不得读取。用户明确询问刚刚、刚才、最近群里或前几条群消息时，生成请求必须排除普通 addressed conversation history，并先读取现场；当前页证据不足且 `next_cursor` 非空时按需继续分页，禁止遍历建立永久群成员档案。历史读取只返回 generation-local 不透明 `image_ref`，不自动下载或识别图片；只有视觉细节影响当前回答时才把某一个精确引用传给 `describe_channel_image`，只需文字或发送原图时不得先识别。`prepare_channel_image` 将本轮历史工具或头像描述工具签发的引用转换为 `media_ref`，再由 `send_msg` 发送。群消息、图片描述与工具结果不得进入 evaluator、画像或长期记忆，也不得向用户暴露 `participant_ref`、cursor、`image_ref`、平台 ID、头像 URL 或数据库字段。
- `llm_chat` 群聊感知工具模块固定为 `tools/find_channel_participants.py`、`tools/read_channel_messages.py`、`tools/describe_channel_image.py`、`tools/describe_channel_participant_avatar.py` 与 `tools/prepare_channel_image.py`；工具只能访问当前 generation-local `Session` 所在账号与公开频道，返回内容始终视为不可信引用数据。`read_channel_messages` 仅按数据库记录签发有限图片引用，真正的协议消息回查、下载与视觉调用推迟到模型选择某个引用后执行。`prepare_channel_image` 只接受本轮历史工具或头像描述工具实际签发的 `image_ref`，按需重新下载 JPEG / PNG / WebP / GIF 并登记内联字节供 `send_msg` 使用；引用不可猜测、跨轮复用、持久化或对用户展示。非空姓名查询先使用已观察身份，并在协议支持时按需扫描有界当前成员目录、仅持久化匹配项；协议不支持且本地无结果时必须返回工具失败，不得把能力缺失伪装成“群里没有”。
...
- `llm_chat` 工具按职责拆分在 `plugins/llm_chat/tools/`：`send_msg.py` 是模型控制的统一发送入口，`_message_models.py` 定义严格的 text/mention/link/emoji/media/break/style 判别联合；`prepared_media.py` 管理 generation-local、有界、单次使用的资源与确认回调，轮次关闭释放未消费资源。`prepare_image`、`prepare_channel_image`、`prepare_external_media`、`prepare_audio`、`synthesize_speech`、`prepare_artifact` 和 `prepare_merged_forward` 仅准备资源；生图、编辑、截图、三类渲染和网页发布同样返回资源供显式组合。`list_prepared_media` 只列本轮未消费资源，不发送。旧的独立发送工具不保留别名。频道图片、参考图授权、注册、provider 与目录继续保持窄模块边界，`tool_runtime.py` 只装配和注册。公开外部资源使用固定公网 DNS、逐跳验证、无代理、无凭证、无本地路径下载；图片只允许 6 MiB 内 JPEG/PNG/WebP/GIF，其他媒体上限 10 MiB，不能绕过频道图片授权。三类渲染继续采用 480–1200 像素、30 秒、6 MiB 输出及 50,000 字符输入边界，`_render_policy.py` 禁止脚本、事件属性和任意资源，`_report.py` 校验固定报告数据；`jinja2pic` 不接受任意模板或路径。
- `generate_image` 使用 `image_generation_model` 指定的独立图像模型：无参考的原创生图走 LiteLLM Images API，选用角色参考时走 Images Edit API 并传入真实图片字节与高输入保真；这只是供应商接口分流，不把新图误作保留原构图的编辑。`edit_image` 将当前用户源图固定为第一张输入，随后附上本轮 `capture_web_reference` 签发的网页参考图及显式选用的角色参考图。网页识图只用于核验参考是否匹配，不替代原始像素；用户指定真实网页参考时仍必须先捕获，再编辑与确认发送，不允许文字重构或原生图片冒充。角色参考与输出使用现有私有审计附件记录，不向模型或公开接口泄露路径、base64 或附件引用。两种工具都只准备 `media_ref`，由 `send_msg` 显式发送；质量、超时、三种尺寸与媒体额度仍由原有边界控制，无参考生图的格式和压缩配置保留，带图请求沿用编辑接口的返回契约。现成表情、直接图片 URL、网页截图和确定性报告渲染继续使用各自工具。
- `edit_image.source_image_index` 固定对应原入站消息 direct-first / quoted-second 的一基序号；附件捕获失败或限额省略不得把后面的图片重编号。运行边界按附件保留的原始 `index` 查找，缺失源图明确拒绝，不能以另一张已捕获图片代替。
...
- 外部参考编辑轮次在编辑结果确认发送前，`send_msg` 仅允许包含本轮已准备编辑结果的消息链；同链可带必要文字，不允许用普通图片或文本单独冒充完成。拒绝发生在额度预留前。`capture_web_reference` 与 `edit_image` 按模型顺序串行，编辑成功只表示准备完成，`send_msg` 获得真实回执才更新 edit_confirmed；Chat Completions 原生图片仍须抑制。
