...
- `llm_chat` 群聊感知边界：当前轮身份通过 `channel_perception` 解析到统一 `person_id`，若绑定变化则按明确旧 ID 列表迁移关系、画像、记忆和用户历史。主聊天默认只自动注入同频道近期 8 条、未指向 Bot、非命令、非 Bot 的有界 `ambient_channel_context`，该上下文不得进入 evaluator、画像或长期记忆；其中历史图片按 `channel_message_max_described_images` 限幅生成视觉描述和 generation-local 不透明 `image_ref`。用户明确询问刚刚、刚才、最近群里或前几条群消息时，生成请求必须排除普通 addressed conversation history，以现场上下文为首要证据；用户指定更早范围或当前页证据不足时，必须调用 `read_channel_messages` 并用非空 `next_cursor` 继续分页，默认 8 条不是历史读取硬上限。按姓名查人必须先取得当前频道不透明 `participant_ref`；不得向用户暴露该引用、cursor、`image_ref`、平台 ID、头像 URL 或数据库字段。`describe_channel_participant_avatar` 按需刷新并描述当前头像像素，按图片字节哈希缓存，并在当前 generation 内返回可交给 `send_channel_image` 的不透明引用；后续轮需要发送时重新查人和描述以取得新引用，不得声称头像只能描述或把 URL 复制给 `send_external_image`。
- `llm_chat` 群聊感知工具模块固定为 `tools/find_channel_participants.py`、`tools/read_channel_messages.py`、`tools/describe_channel_participant_avatar.py` 与 `tools/send_channel_image.py`；工具只能访问当前 generation-local `Session` 所在账号与公开频道，返回内容始终视为不可信引用数据。`send_channel_image` 只接受本轮 `ambient_channel_context`、历史工具或头像描述工具实际签发的 `image_ref`，按需重新下载 JPEG / PNG / WebP / GIF 并以内联字节发送；引用不可猜测、跨轮复用、持久化或对用户展示。非空姓名查询先使用已观察身份，并在协议支持时按需扫描有界当前成员目录、仅持久化匹配项；协议不支持且本地无结果时必须返回工具失败，不得把能力缺失伪装成“群里没有”。自动现场已足够时不得额外读取历史；分页必须服务于当前请求，但只要 `next_cursor` 非空且证据仍不足就允许继续读取，禁止遍历建立永久群成员档案。
- `llm_chat` 工具模块分层：所有 LLM tool 实现位于 `plugins/llm_chat/tools/`，按工具名拆为 `send_image.py`、`send_external_image.py`、`send_channel_image.py`、`send_text.py`、`send_merged_forward.py`、`find_channel_participants.py`、`read_channel_messages.py`、`describe_channel_participant_avatar.py`、`list_image_resources.py`、`list_tts_voices.py`、`send_audio.py`、`speak.py`、`call_plugin.py`、`tag_image.py`、`get_local_time.py`、`web_search.py` 与 `read_web_page.py`；`channel_images.py` 只承载 generation-local 图片引用授权与历史图片视觉富化，共享交付、目录、注册和 provider context 仅放在对应窄职责模块。`tool_runtime.py` 只负责配置、依赖装配、确定注册顺序和汇总 `registered_tools`；人工表情收藏命令独立放在 `meme_command.py`。`send_external_image` 只接受经公开 URL 校验的 HTTP(S) 直接图片地址或经 MIME 嗅探、6 MiB 限幅的 JPEG / PNG / WebP / GIF base64，不读取任意本地路径，也不持久化来源；头像与历史群图片不得通过它绕过 `send_channel_image` 的本轮授权。`get_local_time` 默认读取宿主本地时区并支持显式 IANA timezone。
...
- `llm_chat` 工具执行记忆：Agno 工具桥通过 generation-local `ToolTraceRecorder` 统一记录脱敏后的有效参数、执行状态、confirmed-delivery 效果、限幅结果摘要与时间；轮次结束后写入独立的 `chat_tool_executions`，不得混入普通 `Conversation`、关系 evaluator、画像或长期记忆。下一轮只加载仍位于 channel history window 内的最近事件，并按 `tool_context_max_events` / `tool_context_max_chars` 生成 `runtime_context.recent_tool_activity`；`failed` / `rejected` / `cancelled` 不得声称取得结果，只有 `confirmed` 表示用户可见副作用已确认。网页来源、摘要和正文始终是不可信且可能过时的证据；涉及当前或最新状态时重新核实。不得持久化或回放 Token、Cookie、Authorization、base64/data URL、本地或注册图片路径、完整命令参数、内部数据库信息与 chain-of-thought。
...
- `llm_chat` 模型 I/O：普通主聊天、明确媒体请求与 evaluator 必须分别使用 `model_request_timeout` / `media_request_timeout` / `eval_request_timeout`；明确媒体请求默认使用 300 秒单次超时并关闭 LiteLLM 自动重试，避免上游卡顿被放大为多轮超时。最新用户轮次可直接点名图片、头像、语音等媒体，也可在最近四条上下文已明确媒体对象时用“你能发出来吗”等承接式表达；后者必须进入同一媒体交付与纠正路径，普通代码、文字或文件上下文不得误判。evaluator 依靠严格 JSON prompt 与本地解析，不强制供应商 JSON Mode。未发生发送尝试时，空回复、内部媒体记录或孤立结束标记只允许一次无工具纠正重试；仍失败则精确删除本轮 user 行并跳过 evaluator。用户最新一轮明确或承接式要求发送、生成或补发媒体而 `DeliveryState` 尚未确认媒体发送时，生成层必须在同一网页与交付预算内执行一次保留工具能力的纠正；只有媒体真实发送成功，或模型以内部 `[MEDIA_UNAVAILABLE]` marker 明确本轮不可发送时才可继续，marker 在最终发送和持久化前清除，重复的虚假交付声明视为生成失败。构造 prompt 时不得原样回放持久化媒体 marker：语音仅保留去控制标签后的自然文本，纯表情记录省略。
...
- `llm_chat` 图片资源白名单：自动标注、数据库目录读取、语义检索与 `send_image` 精确路径发送只允许 `resources/image/memes/**`；其他一级目录即使残留文件或 `chat_image_tags` 旧记录也必须被忽略，`..` 逃逸路径同样拒绝。历史 `resources/image/fox_img` 资源已从仓库与生产环境删除，数据库对应索引需清理；`plugins/poke` 不再读取或发送该目录图片，只保留文字、预录音频和反向戳一戳。
- `llm_chat` 表情选择去重：`send_image` 的语义与随机检索必须先排除当前频道 recent window 内已发送的资源，仅当图库没有任何未近期发送资源时才允许回退复用；“别的”“换一张”“不同的”等更换意图按随机换图处理。模型收到明确换图要求时必须使用 `context` 重新检索，禁止用 `image_paths` 指回最近图片或在同一轮重复同一路径。带清晰文字的表情标签必须包含关键原文、具体玩梗含义与 `文字表情包`，不得只按人物外观或泛化情绪标注。
