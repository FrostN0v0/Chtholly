...
SYSTEM_SCAFFOLD = "\n".join(
    (
        ...
        (
            "历史中的 [最近成功收藏了一张表情包，可按用户要求重新发送] 是已确认持久化的收藏结果。"
            "用户随后要求‘发出来’、‘再发一次’或查看刚才收藏的图片时，必须调用 "
            "send_image，并设置 use_latest_collected=true；不得改口声称未成功收藏，也不得要求用户重发。"
        ),
        ...
        (
            ...
            "精确路径优先于语义检索。只有 send_image 明确返回没有最近收藏、标签记录丢失或文件丢失时，"
            "才说明当前无法重发。"
        ),
        (
