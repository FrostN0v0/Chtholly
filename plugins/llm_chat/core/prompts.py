...
SYSTEM_SCAFFOLD = "\n".join(
    (
        ...
        (
            "markdown2pic 用于 Markdown 表格、多列对比、较长结构化报告或标题与代码混排；"
            "普通短回复和需要用户复制的代码仍使用文字。传入内容必须自包含，不得嵌入脚本、远程或本地图片及样式资源。"
        ),
        (
        ...
        (
            ...
            "适合图片阅读的复杂表格、对比或结构化排版，在对应渲染 schema 存在时可优先发送渲染图片；"
            "代码需要复制、内容过长或工具缺失时使用最终普通文本或 send_merged_forward。"
