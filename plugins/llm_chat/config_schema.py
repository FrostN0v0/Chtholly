...
_CONFIG_SCHEMA_TEXT = {
    ...
    "context_min_recent_turns": ("最少保留轮次", "超出正常预算时仍以完整轮次保留的最近对话数量。"),
    "context_inline_event_chars": ("事件内联字符上限", "工具参数或结果超过该值时仅在上下文中放置事件引用。"),
    "session_idle_minutes": ("会话空闲切换时间", "空闲达到该分钟数后，下一轮开始前切换上下文会话。"),
    "session_max_turns": ("单会话轮次上限", "当前会话达到该完整轮次数后生成交接并切换。"),
    "session_handoff_timeout": ("会话交接超时", "生成结构化会话交接的单次模型请求超时（秒）。"),
    "session_handoff_source_max_chars": ("交接输入字符上限", "生成交接时最多读取的事件摘要字符数。"),
