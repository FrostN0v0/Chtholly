...
gsub_weather = on_command("群天气订阅", priority=5)
lookup_p_sub = on_command("查询天气订阅", priority=5)
lookup_g_sub = on_command("查询群天气订阅", priority=5)
del_p_sub = on_command("删除天气订阅", priority=5)
del_g_sub = on_command("删除群天气订阅", priority=5)
...
@sub_weather.handle()
async def sub(event: MessageEvent, arg: CommandArg):
    args = arg.extract_plain_text().split()
    res = await search_city(str(args[0]))
    res = res.json()
    if res["code"] != "200":
        await sub_weather.finish("未获取到城市信息，请检查命令格式或城市名是否正确")
    city_name = res["location"][0]["name"]
    uid = str(event.senderUin)
    ...
    if new_data not in json_dict[uid]:
        ...
        await sub_weather.finish("请勿重复订阅喵~")
    json_dict.save()
    if len(args) > 1:
        scheduler.add_job(push_privite_msg, 'cron', hour=args[1], id=f"weather_{uid}_{city_name}_{args[1]}",
                          kwargs={'uid': uid, 'city': city_name})
        await sub_weather.finish(f"订阅{city_name}天气推送成功，将于每日{args[1]}时推送实时天气")
    else:
        scheduler.add_job(push_privite_msg, 'cron', hour=8, id=f"weather_{uid}_{city_name}_8",
                          kwargs={'uid': uid, 'city': city_name})
        await sub_weather.finish(f"订阅{city_name}天气推送成功，将于每日8时推送实时天气")


@gsub_weather.handle()
async def gsub(event: GroupMessageEvent, arg: CommandArg):
    args = arg.extract_plain_text().split()
    gid = str(event.peerUin)
    uid = str(event.senderUin)
    res = await search_city(str(args[0]))
    res = res.json()
    if res["code"] != "200":
        await gsub_weather.finish("未获取到城市信息，请检查命令格式或城市名是否正确")
    city_name = res["location"][0]["name"]
    ...
    if data_dict not in gsub_data[gid][uid]:
        ...
        await gsub_weather.finish("请勿重复订阅喵~")
    gsub_data.save()
    msg = MessageSegment.text("群用户") + MessageSegment.at(event.senderUin)
    if len(args) > 1:
    ...
    await gsub_weather.finish(message=msg)


@lookup_p_sub.handle()
async def send_p_subs(event: MessageEvent):
    uid = str(event.senderUin)
    msg = MessageSegment.at(uid)
    msg += '您的天气推送订阅列表如下喵~：\n'
    if uid in json_dict:
    ...
    await lookup_p_sub.finish(message=msg)


@lookup_g_sub.handle()
async def send_g_subs(event: GroupMessageEvent):
    uid = str(event.senderUin)
    gid = str(event.peerUin)
    msg = MessageSegment.at(uid)
    ...
    if gid in gsub_data:
        if uid in gsub_data[gid]:
            for i, data_dict in enumerate(gsub_data[gid][uid]):
                ...
                hour = data_dict['hour']
                msg += f"{i+1}.城市：{location}  推送时间：{hour}\n"
    await lookup_g_sub.finish(message=msg)


@del_p_sub.handle()
async def del_p_subs(event: MessageEvent,  arg: CommandArg):
    args = arg.extract_plain_text().split()
    uid = str(event.senderUin)
    msg = "删除订阅成功喵~\n本次删除天气订阅如下：\n"
    if len(args) == 1:
    ...
    await del_p_sub.send(message=msg)
    await del_p_sub.finish(message=msg2)


@del_g_sub.handle()
async def del_g_subs(event: GroupMessageEvent,  arg: CommandArg):
    args = arg.extract_plain_text().split()
    uid = str(event.senderUin)
    gid = str(event.peerUin)
    msg = MessageSegment.at(uid)
    ...
    if gid in gsub_data:
        if uid in gsub_data[gid]:
            for i, data_dict in enumerate(gsub_data[gid][uid]):
                ...
                hour = data_dict['hour']
                msg2 += f"{i+1}.城市：{location}  推送时间：{hour}\n"
    await del_g_sub.send(message=msg)
    await del_g_sub.finish(message=msg2)
