...
async def get_ai_voice(text, voice_type=0) -> Optional[BytesIO]:
    mp3_url = await get_ai_voice_url(text, voice_type)
    if not mp3_url:
        return None
...
async def get_ai_voice_url(text, voice_type=0) -> str:
...
async def split_voice(audio_input) -> Optional[BytesIO]:
