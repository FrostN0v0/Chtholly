...
def event_track_random_location(race, targets):
    for i in targets:
        move_to = random.randint(config.setting_random_min_length, config.setting_random_max_length)
        race.player[i].location_move_to_event(move_to)
...
def event_add_buff(race, targets, buff_name, rounds, buffs, move_min, move_max, event_in_buff):
    for i in targets:
        if race.player[i].find_buff(buff_name) == False:
            logger.info(
                f'{race.player[i].horse} 增加了 buff: {buff_name}, 第{str(race.round + 1)}~{str(race.round + rounds)}回合')
        else:
            race.player[i].del_buff(buff_name)
        race.player[i].add_buff(buff_name, race.round + 1, race.round + rounds, buffs, move_min, move_max,
                                event_in_buff)
