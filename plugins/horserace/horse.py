...
class horse:
    ...
    def location_move(self):
        if self.location != config.setting_track_length:
            self.location_add = self.move() + self.location_add_move
            self.location += self.location_add
            if self.location > config.setting_track_length:
                self.location_add -= self.location - config.setting_track_length
                self.location = config.setting_track_length
            if self.location < 0:
    ...
    def move(self):
        ...
        return random.randint(move_min + config.base_move_min, move_max + config.base_move_max)
    ...
    def display(self):
        ...
        if self.find_buff("hiding") == False:
            if self.location_add < 0:
                ...
                display += "[+" + str(self.location_add) + "]"
            for i in range(0, config.setting_track_length - self.location):
                display += "."
            display += self.horse_fullname
            for i in range(config.setting_track_length - self.location, config.setting_track_length):
                display += "."
        else:
            display += "[+？]"
            for i in range(0, config.setting_track_length):
                display += "."
        display += "\n"
        return display
