from kirami import on_fullmatch, on_time, logger
...
async def daily(matcher: Matcher):
    msg = MessageSegment.image(IMAGE_DIR/"daily60s_img"/f'{date.today()}.png')
    await matcher.finish(msg)


@daily_download.handle()
async def rb_download(matcher: Matcher):
    response = await Request.get("https://api.qqsuu.cn/api/dm-60s")
    with open(IMAGE_DIR / "daily60s_img" / f"{date.today()}.png", 'wb') as f:
        f.write(response.content)


@on_time("interval", minutes=30)
...
async def push(bot: Bot):
    for gid in json_dict['group_list']:
        msg = MessageSegment.image(IMAGE_DIR / "daily60s_img" / f'{date.today()}.png')
        await bot.call_api("send_group_msg", target=gid, message=msg)
